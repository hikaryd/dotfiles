// Stretchy cursor warp adapted from sahaj-b/ghostty-cursor-shaders (MIT).
// Copyright (c) 2026 Sahaj Bhatt. See THIRD_PARTY.md in this directory.
//
...
// --- CONFIGURATION ---
vec4 TRAIL_COLOR = vec4(sRGBToLinear(iCursorColor), 0.34);
const float DURATION = 0.34; // deliberately slower than the upstream preset
const float TRAIL_SIZE = 0.72;
const float THRESHOLD_MIN_DISTANCE = 0.30;
const float BLUR = 1.35;
const float TRAIL_THICKNESS = 0.74;
const float TRAIL_THICKNESS_X = 0.82;

const float FADE_ENABLED = 1.0;
const float FADE_EXPONENT = 1.8;
