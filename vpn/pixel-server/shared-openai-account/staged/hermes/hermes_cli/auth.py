...
from hermes_cli.shared_account_guard import filter_shared_account_pool_entries
...
@contextmanager
def _shared_codex_pool_owner_transaction():
    """Lock the actual managed pool owner, including borrowed-root shadow cases."""
    provider_id = "openai-codex"
    with _auth_store_lock(), ExitStack() as locks:
        local = _load_auth_store()
        local_pool = local.get("credential_pool")
        local_rows = local_pool.get(provider_id) if isinstance(local_pool, dict) else None
        if isinstance(local_rows, list) and local_rows:
            yield local, _auth_file_path()
            return
        root_path = _global_auth_file_path()
        # A local singleton dictionary does not imply ownership of borrowed pool
        # rows. Inspect the root under its lock before following singleton source.
        if not (isinstance(local_rows, list) and local_rows) and root_path is not None:
            locks.enter_context(_auth_store_lock(target_path=root_path))
            root = _load_auth_store(root_path)
            if "shared_account" in root:
                yield root, root_path
                return
        _, source_path = _load_provider_state_with_source(local, provider_id)
        owner_path = source_path or _auth_file_path()
        if not _same_path(owner_path, _auth_file_path()):
            locks.enter_context(_auth_store_lock(target_path=owner_path))
        yield _load_auth_store(owner_path), owner_path
    ...
    *status_cleared_ids* were cleared deliberately (``hermes auth reset``) and skip the
    ...
    with _auth_store_lock():
        ...
        sanitized = [
            ...
            for e in entries]
        sanitized = filter_shared_account_pool_entries(provider_id, sanitized, auth_store)
        ...
        if provider_id == "openai-codex" and "shared_account" in auth_store:
            # Only the shared owner changes lifecycle; cached pools may update status.
            removed.difference_update(existing_by_id)
