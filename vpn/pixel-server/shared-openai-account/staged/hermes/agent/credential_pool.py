...
from hermes_cli.shared_account_guard import (
    filter_shared_account_pool_entries, prepare_shared_account_rotation,
    shared_account_canonical_entry,
)
...
def _update_root_pool_rows(
    ...
    *, status_cleared_ids: Optional[Iterable[str]] = None,
    ...
    with _auth_store_lock(target_path=global_path):
        ...
        payloads = filter_shared_account_pool_entries(provider, payloads, store)
...
class CredentialPool:
    ...
    def _refresh_shared_codex_entry(
        self, entry: PooledCredential, *, force: bool,
    ) -> Tuple[bool, Optional[PooledCredential]]:
        """Managed Codex: owning-store lock spans adopt, POST, and atomic commit."""
        if self.provider != "openai-codex":
            return False, None
        # Native transaction locks the active store then its actual fallback root.
        # Keep this context open through the POST: profile-local locks alone do not
        # serialize single-use grants borrowed from a shared root.
        with auth_mod._shared_codex_pool_owner_transaction() as (store, owner_path):
            if "shared_account" not in store:
                return False, None
            canonical = shared_account_canonical_entry(store)
            if canonical is None:
                return True, None
            synced = PooledCredential.from_dict(self.provider, canonical)
            changed = any(getattr(synced, key) != getattr(entry, key)
                          for key in ("id", "access_token", "refresh_token"))
            self._replace_entry(entry, synced)
            if not self._entry_needs_refresh(synced) and (changed or not force):
                return True, synced
            try:
                updated = replace(self._post_tokens_refresh(synced), **_MARK_OK)
            except Exception as exc:
                # Do not run legacy profile persistence/quarantine paths against
                # a shared owner's grant. Never log provider exception payloads.
                logger.warning("Shared Codex refresh failed (%s)", type(exc).__name__)
                return True, None
            # Re-read and CAS even under the lock: no cached whole-store write can
            # clobber a same-account re-login or a changed generation/pair.
            current = _load_auth_store(owner_path)
            if current.get("shared_account") != store.get("shared_account"):
                raise RuntimeError("Shared Codex account changed during refresh")
            committed = prepare_shared_account_rotation(current, canonical, updated.to_dict())
            _save_auth_store(committed, target_path=owner_path)
            updated = PooledCredential.from_dict(
                self.provider, committed["credential_pool"][self.provider][0],
            )
            self._replace_entry(synced, updated)
            return True, updated

    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        handled, shared = self._refresh_shared_codex_entry(entry, force=force)
        if handled:
            return shared
