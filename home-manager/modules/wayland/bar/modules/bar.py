    ...
    def __init__(self, **kwargs):
        ...
        self.button_config = Button(
            ...
            on_clicked=lambda *_: exec_shell_command_async(
                f'python {data.HOME_DIR}/.config/Ax-Shell/config/config.py'
            ),
        ...
        self.bar_inner = CenterBox(
            ...
            start_children=Box(
                ...
                children=[
                    self.button_apps,
                    Box(
                        name='workspaces-container', children=[self.workspaces]
                    ),
    ...
    def colorpicker(self, button, event):
        if event.button == 1:
            exec_shell_command_async(
                f'bash {get_relative_path("../scripts/hyprpicker-hex.sh")}'
            )
        elif event.button == 2:
            exec_shell_command_async(
                f'bash {get_relative_path("../scripts/hyprpicker-hsv.sh")}'
            )
        elif event.button == 3:
            exec_shell_command_async(
                f'bash {get_relative_path("../scripts/hyprpicker-rgb.sh")}'
            )
