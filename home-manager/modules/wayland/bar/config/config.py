...
SOURCE_STRING = """
# Ax-Shell
source = ~/dotfiles/home-manager/modules/wayland/bar/config/hypr/ax-shell.conf
"""

CONFIG_DIR = os.path.expanduser('~/dotfiles/home-manager/modules/wayland/bar')
WALLPAPERS_DIR_DEFAULT = os.path.expanduser(
...
def ensure_matugen_config():
    ...
    expected_config = {
        ...
        'templates': {
            'hyprland': {
                'input_path': '~/dotfiles/home-manager/modules/wayland/bar/config/matugen/templates/hyprland-colors.conf',
                'output_path': '~/dotfiles/home-manager/modules/wayland/bar/config/hypr/colors.conf',
            },
            'ax-shell': {
                'input_path': '~/dotfiles/home-manager/modules/wayland/bar/config/matugen/templates/ax-shell.css',
                'output_path': '~/dotfiles/home-manager/modules/wayland/bar/styles/colors.css',
    ...
    image_path = os.path.expanduser(
        '~/dotfiles/home-manager/modules/wayland/bar/assets/wallpapers_example/example-1.jpg'
    )
...
def ensure_fonts():
    ...
    fonts_to_copy = [
        (
            '~/.fonts/zed-sans/',
            '~/dotfiles/home-manager/modules/wayland/bar/assets/fonts/zed-sans/',
        ),
        (
            '~/.fonts/tabler-icons/',
            '~/dotfiles/home-manager/modules/wayland/bar/assets/fonts/tabler-icons/',
        ),
    ]
...
def generate_hyprconf() -> str:
    ...
    return f"""exec-once = uwsm app -- python {home}/dotfiles/home-manager/modules/wayland/bar/main.py
exec = pgrep -x "hypridle" > /dev/null || uwsm app -- hypridle
exec = uwsm app -- swww-daemon

$fabricSend = fabric-cli exec ax-shell
$axMessage = notify-send "Axenide" "What are you doing?" -i "{home}/dotfiles/home-manager/modules/wayland/bar/assets/ax.png" -a "Source Code" -A "Be patient. 🍙"

bind = {bind_vars['prefix_restart']}, {bind_vars['suffix_restart']}, exec, killall ax-shell; uwsm app -- python {home}/dotfiles/home-manager/modules/wayland/bar/main.py # Reload Ax-Shell | Default: SUPER ALT + B
...
source = {home}/dotfiles/home-manager/modules/wayland/bar/config/hypr/colors.conf
...
def ensure_face_icon():
    ...
    default_icon_path = os.path.expanduser(
        '~/dotfiles/home-manager/modules/wayland/bar/assets/default.png'
    )
...
class HyprConfGUI(Gtk.Window):
    def __init__(self, show_lock_checkbox: bool, show_idle_checkbox: bool):
        ...
        for label_text, prefix_key, suffix_key in bindings:
            ...
            self.entries.append(
                (prefix_key, suffix_key, prefix_entry, suffix_entry)
            )
        ...
        if show_lock_checkbox:
            self.lock_checkbox = Gtk.CheckButton(
                label='Replace Hyprlock config'
            )
            self.lock_checkbox.set_active(False)
            main_vbox.pack_start(self.lock_checkbox, False, False, 0)
        if show_idle_checkbox:
            self.idle_checkbox = Gtk.CheckButton(
                label='Replace Hypridle config'
            )
    ...
    def on_accept(self, widget):
        ...
        if self.selected_face_icon:
            try:
                ...
                cropped_img.save(
                    os.path.expanduser('~/.face.icon'), format='PNG'
                )
        ...
        if hasattr(self, 'lock_checkbox') and self.lock_checkbox.get_active():
            src_lock = os.path.expanduser(
                '~/dotfiles/home-manager/modules/wayland/bar/config/hypr/hyprlock.conf'
            )
        ...
        if hasattr(self, 'idle_checkbox') and self.idle_checkbox.get_active():
            src_idle = os.path.expanduser(
                '~/dotfiles/home-manager/modules/wayland/bar/config/hypr/hypridle.conf'
            )
        ...
        # hyprland_config_path = os.path.expanduser(
...
def start_config():
    ...
    hypr_config_dir = os.path.expanduser(
        '~/dotfiles/home-manager/modules/wayland/bar/config/hypr/'
    )
